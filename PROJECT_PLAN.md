...
- 已完成 React/TypeScript/Vite 前端骨架、内容页、认证页面、受保护路由和启动时会话恢复；
- 已完成真实 JPEG/PNG 上传、服务端解码/哈希校验、模拟任务提交和工作台最近任务轮询；
- 已完成上传单张删除、用户取消、取消审计和超时目录回收；
- 已完成任务详情/取消与带认证 SSE 进度，详情仅返回契约校验后的结果和评估摘要；
- 下一步接入真实 Re3D 执行控制器与产物授权下载。
...
当前进度：React/Vite 与 FastAPI 骨架、PostgreSQL/Alembic 和基础测试已经完成；统一错误响应、OpenAPI 类型生成和 CI 尚未完成。
...
当前进度：认证 API、令牌轮换、对象级任务隔离及注册/登录/退出/恢复页面已经完成；邮箱验证、密码重置、限流和安全审计尚未完成。
...
当前进度：上传会话、JPEG/PNG 校验、单张删除、上传取消/过期清理、不可变 manifest、模拟任务提交、数据库租约、任务详情/取消、带认证 SSE 和轮询降级已经完成。真实与模拟消费者按 execution mode 隔离；模拟长任务恢复的专项验收仍待补充。
...
当前进度：已实现显式 real 入队、受租约保护的 Re3D 进程控制、Windows/Linux 进程树终止、总超时、步骤事件、三分支标准产物哈希归档和结果幂等复用，并通过轻量假管线闭环测试。2026-09-25 已完成 11 图本机真实 GPU 闭环验收；资源采样、失败目录自动清理和授权下载尚未完成。
