...
| `/workspace` | 重建工作台 | 受保护页面；分批上传、删图、取消、提交模拟任务并轮询最近任务 |
| `/workspace/jobs/:jobId` | 任务详情 | 受保护页面；实时进度、任务取消、三分支结果摘要和自动评估摘要 |

工作台上传真实 JPEG/PNG，但明确标记当前任务仍为模拟执行，不把模拟 GLB 表述为真实重建结果。任务详情不直接接收服务器文件路径，只消费后端经过所有权校验和契约裁剪的摘要。
...
- SSE 使用带 Bearer 请求头的 Fetch 流，不在查询参数、浏览器历史或代理日志中放置 access token；连接不可用时以 5 秒轮询降级。

认证后的 API 客户端在 access token 过期后合并 refresh 请求并只重试原请求一次，避免并发轮换和无限重试。
...
- 已实现分批上传、单张删除、上传取消、提交、最近任务轮询、任务详情/取消、SSE 状态流和评估摘要；尚未实现刷新后恢复未提交会话、产物授权下载和 GLB 查看器；
